...
<body>
    ...
    <div class="jumbotron">
        <h2 class="display-5">Caixa</h2>
        <h3 class="display-8">Seleccionar dates</h3>
        <?php //echo form_open('recibos/arqueo', array('class' => "form-horizontal", 'role' => "form")); 
        ?>
        ...
        <button type="text" id="arqueoCaja" class="btn btn-primary">Arqueig de caixa</button>
        <?php //echo form_close(); 
        ?>
...
<script>
    $(document).ready(function() {
        ...
        $('#arqueoCaja').click(function() {
            var desde = $('#desde').val()
            var hasta = $('#hasta').val()
            if (!desde) desde = "01/09/2019"
            if (!hasta) hasta=hoy()

            desde = desde.substring(6) + '-' + desde.substring(3, 5) + '-' + desde.substring(0, 2);
            hasta = hasta.substring(6) + '-' + hasta.substring(3, 5) + '-' + hasta.substring(0, 2);
            ...
            var url = "<?php echo base_url() ?>index.php/recibos/arqueo/" + desde + "/" + hasta
            // alert(url)
